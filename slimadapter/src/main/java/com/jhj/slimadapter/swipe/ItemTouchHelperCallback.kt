package com.jhj.slimadapter.swipe
...
class ItemTouchHelperCallback(private val mAdapter: SlimAdapter) : ItemTouchHelper.Callback() {
    ...
    override fun onSelectedChanged(viewHolder: RecyclerView.ViewHolder?, actionState: Int) {
        viewHolder?.let {
            if (actionState == ItemTouchHelper.ACTION_STATE_DRAG && isBodyViewHolder(it)) {
                dragListener?.onItemDragStart(it, getViewHolderPosition(it))
                isDragging = true
            } else if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE && isBodyViewHolder(it)) {
                swipeListener?.onItemSwipeStart(it, getViewHolderPosition(it))
                isSwiping = true
            }
        }
